...
#
"""Executable and reusable sample for updating GCP settings.

In Chronicle, customers can associate their GCP organizations with their
Chronicle instances. After that, customers can control how their logs flow
into Chronicle by GCP settings. This example provides a programmatic way to
update such settings.
"""
...
def initialize_command_line_args(
  ...
  parser.add_argument(
      ...
      type=int,
      required=True,
      help="the GCP organization ID for the settings")
  parser.add_argument("--enable", dest="ingestion", action="store_true")
  parser.add_argument("--disable", dest="ingestion", action="store_false")
  parser.set_defaults(ingestion=True)
