...
#
"""Executable and reusable sample for creating a GCP association.

In Chronicle, customers can associate their GCP organizations with their
Chronicle instances. This example provides a programmatic way to create such
association.
"""
...
def initialize_command_line_args(
  ...
  parser.add_argument(
      ...
      type=str,
      required=True,
      help="access code from Chronicle Customer Representative")
  ...
  if parsed_args.organization_id >= 2**64 or parsed_args.organization_id < 0:
    ...
    return None
  if len(parsed_args.nonce) != 64:
    print("Error: nonce should be of length 64")
    return None
...
if __name__ == "__main__":
  ...
  create_gcp_association(session, cli.organization_id, cli.nonce)
