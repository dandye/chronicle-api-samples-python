...
#
"""Executable and reusable sample for deleting a subject.

A subject can be an analyst or an Identity Provider (IdP) group.

API reference:
https://cloud.google.com/chronicle/docs/reference/rbac-api#deletesubject
"""
...
def initialize_command_line_args(
  ...
  # No need for a sanity check for the subject name because this argument
  # converts the provided input into strings and accept a wide range of values.
  # If the subject name isn't passed in, the error will be thrown from the
  # argparse library.

  return parser.parse_args(args)


def delete_subject(http_session: requests.AuthorizedSession, name: str) -> None:
  """Deletes a subject.

  Args:
    http_session: Authorized session for HTTP requests.
    name: The ID of the subject to be deleted.
...
if __name__ == "__main__":
  ...
  print(json.dumps(delete_subject(session, cli.name), indent=2))
